//! AI Object Header Definition
//!
//! This module defines the common metadata header shared by all AI process objects.
//! It ensures that every object in the AI workflow has a consistent identity,
//! provenance, and integrity verification mechanism.
//!
//! # Core Concepts
//!
//! - **Identity**: Every object has a unique `Uuid` (v7) and `AiObjectType`.
//! - **Provenance**: `ActorRef` tracks who created the object (Human, Agent, System).
//! - **Integrity**: `IntegrityHash` (SHA-256) ensures content has not been tampered with.
//! - **Linking**: Objects reference each other via UUIDs (e.g., Run -> Task).
...
///
/// Enumerates all supported object types in the AI workflow system.
#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
pub enum AiObjectType {
    /// A unit of work to be performed.
    Task,
    /// An execution instance of an agent working on a task.
    Run,
    /// A set of code changes proposed by an agent.
    PatchSet,
    /// A snapshot of the codebase context used for a run.
    ContextSnapshot,
    /// A record of a tool execution (read, write, cmd, etc.).
    ToolInvocation,
    /// A step-by-step plan generated by the agent.
    Plan,
    /// Verification results (test output, lint errors).
    Evidence,
    /// Metadata about the model and provider used.
    Provenance,
    /// The final outcome of a run (commit, reject, etc.).
    Decision,
}
