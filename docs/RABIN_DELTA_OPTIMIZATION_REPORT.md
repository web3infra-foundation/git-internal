...
- `encode_and_output_to_files` 是唯一的 pack/idx 文件输出入口。
- Cargo 默认 feature 为 `default = ["diff_rabin"]`，因此 `window_size > 0` 默认使用 Rabin fingerprint delta。
- `window_size == 0` 不执行 delta，使用有序批量并行编码。
- 关闭 `diff_rabin` 后，启用 `diff_mydrs` 时回退到 Myers；两个 feature 都关闭时回退到 Patience。
- 原 `encode_and_output_to_files_with_rabin` 和 `encode_and_output_to_files_with_rabin_no_prefilter` 已删除，不再通过不同函数选择算法。
- Rabin 默认路径会检查 delta 窗口内全部符合大小、类型和链深限制的候选 base，不使用内容相似度预过滤。
...
2026-06-04  a3c174f   增加 git 官方 benchmark 脚本 (git_pack_objects_bench.sh)
           27e45b8   增加 docs/gitc 参考源码 (diff-delta.c, pack-objects.c, pack-write.c, delta.h)
...
| `Cargo.toml` | 修改 | +15/-2 | `diff_rabin` feature flag、默认 Rabin 选择与 profiling 特性 |
...
### 6.2 git C 参考源码（只读，仅在 docs/gitc/ 目录，不参与编译）

| 文件 | 行数 | 说明 |
|------|------|------|
| `docs/gitc/diff-delta.c` | 510 | git 的 Rabin delta 算法实现 |
| `docs/gitc/pack-objects.c` | 5,430 | git 的 pack 编码器（含多线程） |
| `docs/gitc/pack-write.c` | 614 | git 的 pack 写入逻辑 |
| `docs/gitc/delta.h` | 114 | git 的 delta 接口定义 |
| **docs/gitc 小计** | **6,668** | |
